...
def suggest_stocks(investment, selected_stocks):
    ...
    for i, stock in enumerate(selected_stocks):
        ...
        investment_amounts.append(amount_to_invest)
        labels.append(f"{stock} (${amount_to_invest:.2f})")
        print(f"{stock}: {allocation_percentage*100:.2f}% (${amount_to_invest:.2f})")

    # Create a pie chart
    plt.figure(figsize=(8, 8))
    plt.pie(investment_amounts, labels=labels, autopct='%1.1f%%')
    plt.title('Investment Distribution Among Selected Stocks')
    plt.show()

# Rest of your main function and other functions remain the same
